...
/**
 * Renders the images at the specified indices from the image storage into a none-searchable PDF 
 ...
 * The image storage is copied, it is safe to modify it after enqueuing the operation.
 ...
 */
+ (SBSDKProgress *)renderImageStorage:(SBSDKImageStorage *)imageStorage
                     copyImageStorage:(BOOL)copyStorage
                             indexSet:(NSIndexSet *)indices
                         withPageSize:(SBSDKPDFRendererPageSize)pageSize
                               output:(NSURL *)pdfOutputURL
                    completionHandler:(SBSDKCompletionHandler)completionHandler;
