 ...
 * A mutable ordered collection of disk-backed UIImage objects.
 ...
 * Ranges from 0.0 (small files, bad quality) to 1.0 (huge files, great quality).
 **/
 ...
 **/
+ (instancetype)temporaryStorageWithImages:(NSArray *)images;

/** 
 * Designated initializer. Creates an image storage in the folder specified by  folderURL.
 * @param folderURL An NSURL describing the location of the image storage folder on the disk.
 * @return Initialized image storage.
 **/
 ...
 **/
- (UIImage *)thumbnailForImageAtIndex:(NSUInteger)index ofSize:(CGSize)size;
 ...
 **/
- (NSIndexSet *)validatedIndexSetForIndexSet:(NSIndexSet *)indexSet;
